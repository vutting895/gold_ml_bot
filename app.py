...
st.set_page_config(
    page_title="Gold SMC Trading Dashboard",
    page_icon="🪙",
    layout="wide"
)

# โหลดค่าตัวแปรสภาพแวดล้อม ( Environment Variables / Streamlit Secrets )
GOOGLE_CREDENTIALS_JSON = os.environ.get("GOOGLE_CREDENTIALS_JSON") or st.secrets.get("GOOGLE_CREDENTIALS_JSON")
GOOGLE_SHEET_NAME = os.environ.get("GOOGLE_SHEET_NAME") or st.secrets.get("GOOGLE_SHEET_NAME") or "Gold_Trading_Logs"
...
if not df_signals.empty and "Type" in df_signals.columns:
    ...
    time_col = "Time (UTC+7)" if "Time (UTC+7)" in df_signals.columns else df_signals.columns[0]
